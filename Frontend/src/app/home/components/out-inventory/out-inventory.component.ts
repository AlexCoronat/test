...
@Component({
  ...
  styleUrl: './out-inventory.component.css'
})
export class OutInventoryComponent implements OnInit {
  ...
  onModelChange(newValue: number): void {
    if(this.editingProduct) {
      if(newValue > this.originalProduct.stock && !this.permissions.includes('AI')) {
        this.openRestrictionDialog('No tiene permisos para aumentar el stock');
  ...
  openRestrictionDialog(message: string): void {
    this.dialog.open(ORestrictionDialog, {
      data: { message }
    });
  }
...
export class ORestrictionDialog {
